// ==UserScript==
// @name         Duel Tools — Relay
// @namespace    http://tampermonkey.net/
// @version      3.0
// @description  Sends Duelingbook replay data directly to Duel Tools server
// @author       nedhmn/duel-tools port
// @match        https://www.duelingbook.com/replay*
// @grant        GM_xmlhttpRequest
// @grant        GM_getValue
// @grant        GM_setValue
// @connect      *
// @run-at       document-start
// ==/UserScript==
(function () {
  'use strict';

  const id = new URLSearchParams(window.location.search).get('id');
  if (!id) return;

  // ── Server URL — update this if your Railway URL changes ──────────────────
  const SERVER = GM_getValue('duel_tools_server', 'https://duel-tools-kerem-production.up.railway.app');
  let sent = false;

  function send(rawData) {
    if (sent) return; sent = true;
    const plays = rawData.plays || rawData;
    const allPlays = Array.isArray(plays) ? plays : [];
    const rps = allPlays.filter(p => p.play === 'RPS').map(p => ({
      ...p,
      player1_choice: p.player1_choice || p.p1pick || null,
      player2_choice: p.player2_choice || p.p2pick || null,
    }));

    const payload = {
      replayId: id,
      plays: rps,
      allPlays: allPlays,
      timedOut: false,
      player1: rawData.player1 || null,
      player2: rawData.player2 || null,
      date:    rawData.date    || null,
      format:  rawData.format  || null,
    };

    // Post directly to the server — no iframe, no postMessage needed
    GM_xmlhttpRequest({
      method: 'POST',
      url: SERVER + '/api/relay/replay',
      headers: { 'Content-Type': 'application/json' },
      data: JSON.stringify(payload),
      onload: (r) => console.log('[DuelTools] Relay sent:', r.status, r.responseText.slice(0, 100)),
      onerror: (e) => console.warn('[DuelTools] Relay error:', e),
    });
  }

  function isReplay(v) {
    return v && typeof v === 'object' && !Array.isArray(v) &&
      Array.isArray(v.plays) && v.plays.length > 0 && 'play' in v.plays[0];
  }
  function isPlays(arr) {
    return Array.isArray(arr) && arr.length > 0 && arr[0] &&
      typeof arr[0] === 'object' && 'play' in arr[0];
  }
  function tryValue(v) {
    if (sent) return false;
    if (isReplay(v)) { send(v); return true; }
    if (isPlays(v)) { send({ plays: v }); return true; }
    if (typeof v === 'string' && v.length > 100) {
      try {
        const i = v.indexOf('{');
        if (i >= 0) {
          const p = JSON.parse(v.slice(i));
          if (isReplay(p)) { send(p); return true; }
          if (isPlays(p.plays)) { send(p); return true; }
        }
      } catch(e) {}
    }
    return false;
  }

  // Hook console.log (duelingbook logs replay data here)
  const origLog = console.log.bind(console);
  console.log = function(...a) {
    origLog(...a);
    if (!sent) for (const x of a) if (tryValue(x)) return;
  };

  // Hook XHR
  const OO = XMLHttpRequest.prototype.open;
  const OS = XMLHttpRequest.prototype.send;
  XMLHttpRequest.prototype.open = function(m, u) {
    this._url = String(u || '');
    return OO.apply(this, arguments);
  };
  XMLHttpRequest.prototype.send = function() {
    this.addEventListener('load', function() {
      if (!sent && this._url && this._url.includes('duelingbook'))
        tryValue(this.responseText);
    });
    return OS.apply(this, arguments);
  };

  // Hook fetch
  const oF = window.fetch;
  window.fetch = function(inp, init) {
    const url = typeof inp === 'string' ? inp : (inp && inp.url) || '';
    const p = oF.apply(this, arguments);
    if (url.includes('duelingbook'))
      p.then(r => r.clone().text().then(tryValue).catch(() => {})).catch(() => {});
    return p;
  };

  // Poll window globals
  const t = setInterval(() => {
    if (sent) { clearInterval(t); return; }
    try {
      for (const k of Object.keys(window)) {
        const v = window[k];
        if (isReplay(v) && v.plays.length > 2) { clearInterval(t); send(v); return; }
        if (v && typeof v === 'object' && isPlays(v.plays) && v.plays.length > 2) { clearInterval(t); send(v); return; }
      }
    } catch(e) {}
  }, 300);

  // Timeout after 35s
  setTimeout(() => {
    clearInterval(t);
    if (!sent) {
      sent = true;
      GM_xmlhttpRequest({
        method: 'POST',
        url: SERVER + '/api/relay/replay',
        headers: { 'Content-Type': 'application/json' },
        data: JSON.stringify({ replayId: id, plays: [], allPlays: [], timedOut: true, player1: null, player2: null }),
        onerror: () => {},
      });
    }
  }, 35000);
})();
